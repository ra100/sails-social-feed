...
const status = (status, feedId) => {
  ...
  if (['add', 'edited'].includes(status.verb)) {
    Feed.findOne({ where: { config: feedId, enabled: true } })
      .populate('stream')
      .then(feed => {
        if (!feed) {
        ...
        return Message.findOne({ uuid: uuid }).then(msg => {
          const message = {
            ...
            uuid: uuid,
            created: status.created_time
              ? new Date(status.created_time * 1000)
              : new Date(),
            link: '',
            author: {},
      ...
      .then(message => {
        ...
        return Promise.all([authorPromise, metaPromise, videoPromise]).then(
          values => {
      ...
      .then(message => {
        if (exists) {
          return Message.update({ uuid: message.uuid }, message)
            .then(messages => {
...
const reaction = (status, feedId) =>
  Message.findOne({ uuid: { endsWith: status.post_id } })
    .then(message => {
      if (!message) {
    ...
    .then(message => {
      Message.update(
        { where: { uuid: { endsWith: status.post_id } }, limit: 1 },
        message
  ...
  return new Promise((resolve, reject) => {
    fb.api(`/${id}`, 'get', { fields: ['name', 'picture', 'link'] }, result => {
      ...
      return resolve({
        ...
        id: result.id,
        picture:
          result.picture && result.picture.data ? result.picture.data.url : ''
      })
    })
  })
}
