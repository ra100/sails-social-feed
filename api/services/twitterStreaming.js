...
const twitterStreamingClass = function() {
  ...
  this.processData = data => {
    ...
    if (typeof data.retweeted_status !== 'undefined') {
      ...
      if (typeof this.twitter_user[screen_name] !== 'undefined') {
        ...
        let hashtag_feed = this.findHashtag(
    ...
    if (feed == null) {
      sails.log.debug(
        'Status does not match any feed',
        status_link + status.id_str
      )
      return
    }
  ...
  this.invalidateToken = () => {
    ...
    return Feed.findOne(this.auth_feed_id)
      .then(feed => {
        if (feed) {
          feed.auth.valid = false
          Feed.update(feed.id, { auth: feed.auth })
            .then(feed => {
