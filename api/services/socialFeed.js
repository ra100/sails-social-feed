...
module.exports = {
          ...
          return Passport.create({
            ...
            if (err) {
              if (err.code === 'E_VALIDATION') {
                throw 'Error.Passport.Password.Invalid'
              }
            }
          })
        })
  ...
  authTwitter: function(req, res) {
    ...
    var oa = new oauth.OAuth(
      ...
      '1.0',
      sails.config.baseurl + '/feeds/twitter/' + id,
      'HMAC-SHA1'
    )
            ...
            oa.get(
              ...
              (err, twitterResponseData) => {
                ...
                return Feed.update(id, {
                  ...
                  .then(() => {
                    return twitterStreaming.reconnect(
  ...
  authFacebook: (req, res) => {
    ...
    return res.ok({
      redirect: `https://www.facebook.com/v${
        sails.config.auth.facebook_api_version
      }/dialog/oauth?client_id=${
        sails.config.auth.facebook_app_id
      }&redirect_uri=${
        sails.config.baseurl
      }/feeds/facebook?feed=${id}&scope=manage_pages`
    })
  },
