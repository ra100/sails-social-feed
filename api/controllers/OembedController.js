...
fb.setAccessToken(
  `${sails.config.auth.facebook_app_id}|${
    sails.config.auth.facebook_app_secret
  }`
)
...
const fbvideoPattern = /^https:\/\/www\.facebook\.com\/(?:[^\/]+\/videos\/|video.php\?(?:id|v)=)(\d+)\.*/g
...
const fetchUrl = url => {
  return new Promise((resolve, reject) => {
    const client = new MetaInspector(url, {
      ...
      headers: {
        'User-Agent':
          'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.59 Safari/537.36 Vivaldi/1.7.725.3'
      }
    })
    client.on('fetch', () => {
...
const fetchFacebook = (url, type) =>
  request
    .get(`https://www.facebook.com/plugins/${type}/oembed.json/?url=${url}`)
    .then(res => {
        ...
        (result, err) => {
          ...
          const image =
            (result.attachments &&
              ...
              result.attachments.data[0] &&
              result.attachments.data[0].media &&
              result.attachments.data[0].media.image) ||
            {}
          resolve({
        ...
        (result, err) => {
          ...
          if (result.source && result.source.includes('fbcdn')) {
            ...
            return resolve({
              ...
              height: image.height,
              url: result.permalink_url,
              html: `<iframe src=\"https://www.facebook.com/video/embed?video_id=${
                id.split('_')[1]
  ...
  youtube: function(req, res) {
    ...
    Oembed.findOne({
      ...
      .then(oembed => {
        ...
        return request
          .get('https://www.youtube.com/oembed?format=json&url=' + url)
          .then(result => {
            if (!result.body) {
      ...
      .then(oembed => {
        ...
        return new Promise((resolve, reject) => {
          ...
          engine.getEmbed(embed, res => {
            ...
            saveOembed(url, res.data).catch(sails.log.error)
